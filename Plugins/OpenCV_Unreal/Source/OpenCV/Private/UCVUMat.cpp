...
#define CV_ENABLE_INSTANCE_TRACKING 0
...
void UCVUMat::FromTexture2D(UTexture2D *Texture, UCVUMat *&Mat) {
  ...
  int cvFormat{-1};
  if (Texture->GetPlatformData()->PixelFormat == PF_G8) {
    cvFormat = CV_8UC1;
  ...
  ENQUEUE_RENDER_COMMAND(SceneDrawCompletion)(
    [CommandData](FRHICommandListImmediate& RHICmdList) {
      ...
      cv::Mat memoryWrapper(SizeY, SizeX, CommandData->cvFormat, memoryBuffer, DestPitch);
  ...
  Future.Get();
  //cv::imwrite("E:/1.png", CommandData->Mat);
  UE_LOG(OpenCV, Warning, TEXT("==> type: %d == %d == %d %dx%d"), CommandData->cvFormat, CommandData->Mat.type(), CV_8UC4, CommandData->Mat.cols, CommandData->Mat.rows);
  cv::imshow("!", CommandData->Mat);
  cv::waitKey();
  cv::destroyWindow("!");
